...
void Search::Worker::start_searching() {
    ...
    hordeQsearchWhitePawnChecks = bool(options["HordeQsearchWhitePawnChecks"]);
...
template<NodeType nodeType>
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth qDepth) {
    ...
    MovePicker mp(pos, ttData.move, DEPTH_QS, &mainHistory, &lowPlyHistory, &captureHistory,
                  ...
                  hordeQsearchWhitePawnChecks && qDepth == DEPTH_QS);
    ...
    while ((move = mp.next_move()) != Move::none())
    {
            ...
            && HORDE_PRUNING_ACTIVE(HordeDisableQsearchPruning))
        {
            ...
            // Skip non-captures
            if (!capture
                && !(hordeQsearchWhitePawnChecks && givesCheck
                     && pos.moved_piece(move) == W_PAWN))
            {
#if defined(HORDE_SEARCH_TELEMETRY)
                if (hordeMetrics)
                {
        ...
        value = -qsearch<nodeType>(pos, ss + 1, -beta, -alpha,
                                   hordeQsearchWhitePawnChecks ? DEPTH_QS_NO_CHECKS : DEPTH_QS);
        undo_move(pos, move);
    ...
    ttWriter.write(posKey, value_to_tt(bestValue, ss->ply), pvHit,
                   bestValue >= beta ? BOUND_LOWER : BOUND_UPPER, qDepth, bestMove,
                   unadjustedStaticEval, tt.generation());
