...
SCHEMA = "HORDE_TRAINING_BOOK_SPLIT_V1"
...
def _canonical_lines(payload: bytes) -> list[tuple[bytes, str, str]]:
    try:
    ...
    for line_number, raw_line in enumerate(text.splitlines(), start=1):
        ...
        digest = hashlib.sha256(position_key.encode("ascii")).digest()
        entries.append((digest, position_key, line))

    _require(entries, "source book is empty")
    return entries


def _payload(entries: list[tuple[bytes, str, str]]) -> bytes:
    ordered = sorted(entries, key=lambda entry: (entry[0], entry[1], entry[2]))
    return ("\n".join(entry[2] for entry in ordered) + "\n").encode("ascii")


def split_book(source: Path, modulus: int, validation_residue: int) -> tuple[bytes, bytes, dict[str, object]]:
    ...
    entries = _canonical_lines(source_payload)

    train: list[tuple[bytes, str, str]] = []
    validation: list[tuple[bytes, str, str]] = []
    ...
    train_keys = {entry[1] for entry in train}
    validation_keys = {entry[1] for entry in validation}
    ...
    receipt = {
        "schema": SCHEMA,
        "assignment": {
            "key": "first four normalized FEN fields",
            "hash": "SHA-256",
...
def write_split(
    ...
    train, validation, receipt = split_book(source, modulus, validation_residue)
